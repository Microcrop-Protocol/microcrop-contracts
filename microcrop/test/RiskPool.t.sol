...
contract RiskPoolTest is Test {
    RiskPool public implementation;
    ...
    function _createPublicConfig() internal view returns (RiskPool.PoolConfig memory) {
    ...
    function _createPrivateConfig() internal view returns (RiskPool.PoolConfig memory) {
        return RiskPool.PoolConfig({
            usdc: address(usdc),
            poolId: 2,
            name: "Private Pool",
    ...
    function _createMutualConfig() internal view returns (RiskPool.PoolConfig memory) {
        return RiskPool.PoolConfig({
            usdc: address(usdc),
            poolId: 3,
            name: "Mutual Pool",
    ...
    function _deployPublicPool() internal returns (RiskPool) {
        RiskPool.PoolConfig memory config = _createPublicConfig();
        ...
        return RiskPool(address(proxy));
    }

    function _deployPrivatePool() internal returns (RiskPool) {
        RiskPool.PoolConfig memory config = _createPrivateConfig();
        bytes memory initData = abi.encodeWithSelector(
            RiskPool.initialize.selector,
            config
        );
    ...
    function _deployMutualPool() internal returns (RiskPool) {
        RiskPool.PoolConfig memory config = _createMutualConfig();
        bytes memory initData = abi.encodeWithSelector(
            RiskPool.initialize.selector,
            config
        );

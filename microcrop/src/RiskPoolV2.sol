 ...
 * @notice ERC20 LP token representing fractional ownership of an insurance risk pool
 ...
 */
contract RiskPoolV2 is ERC20, AccessControl, ReentrancyGuard, Pausable {
    ...
    /// @notice Human-readable pool name (e.g., "Kenya Maize Drought Pool")
    ...
    /// @notice Minimum deposit amount (varies by pool type)
    ...
    /// @notice Configuration struct for pool creation
    struct PoolConfig {
        address usdc;
        uint256 poolId;
        string name;
    ...
    /**
     * @notice Create a new RiskPoolV2
     ...
     * @dev Tokens minted = USDC deposited / current token price
     *      New LPs buy in at current NAV, no dilution to existing holders
     */
     ...
     * @dev USDC received = tokens burned * current token price
     *      LPs exit at current NAV, no impact on other holders
     */
    function withdraw(uint256 tokenAmount) external nonReentrant {
        ...
        // Check pool has liquidity (must maintain buffer for claims)
     ...
     * @param distributor Address of the distributor (can be different per policy)
     * @dev Distribution: LP 70%, Builder 12%, Protocol 10%, Distributor 8%
     */
    function collectPremium(
        ...
        // LP share stays in pool (increases token value)
        // Note: lpShare is already in the contract from the transfer above

        // Distribute builder share
        if (productBuilder != address(0) && builderShare > 0) {
            USDC.safeTransfer(productBuilder, builderShare);
        } else {
            // If no builder, goes to LP pool
        ...
        if (actualDistributor != address(0) && distributorShare > 0) {
            ...
            // If no distributor, goes to LP pool
            lpShare += distributorShare;
        }

        // Track total premiums (gross for accounting)
    ...
    /**
     * @notice Update active exposure when policies are created/expired
     * @param policyId Policy identifier
     * @param delta Change in exposure (positive = new policy, negative = expired)
     */
     ...
     * @dev Price = Pool Value / Total Supply
     *      Returns 1e18 (1.00 USDC) if no tokens exist
     */
     ...
     * @dev Reserves 20% buffer above active exposure for claims
     */
    function getAvailableLiquidity() public view returns (uint256) {
        uint256 poolBalance = USDC.balanceOf(address(this));
        uint256 reserved = (activeExposure * 120) / 100; // 120% of exposure
     ...
     * @return roi Return on investment (basis points, can be negative)
     */
    ...
    /**
     * @notice Check if pool can accept a policy with given exposure

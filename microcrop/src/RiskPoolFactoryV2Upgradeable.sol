...
import {RiskPoolV2Upgradeable} from "./RiskPoolV2Upgradeable.sol";

/**
 * @title RiskPoolFactoryV2Upgradeable
 ...
 */
contract RiskPoolFactoryV2Upgradeable is
    Initializable,
    ...
    UUPSUpgradeable
{
    ...
    struct PoolMetadata {
        ...
        uint256 poolId;
        string name;
        RiskPoolV2Upgradeable.PoolType poolType;
        RiskPoolV2Upgradeable.CoverageType coverageType;
        string region;
        address poolOwner;
        uint256 createdAt;
    }
    ...
    /// @notice RiskPoolV2Upgradeable implementation address
    ...
    mapping(RiskPoolV2Upgradeable.PoolType => address[]) public poolsByType;
     ...
     * @param _poolImplementation RiskPoolV2Upgradeable implementation address
     */
    function initialize(
        address _usdc,
        address _treasury,
        ...
        returns (address poolAddress)
    {
        ...
        RiskPoolV2Upgradeable.PoolConfig memory config = RiskPoolV2Upgradeable.PoolConfig({
            usdc: usdc,
            poolId: newPoolId,
            name: params.name,
        ...
        returns (address poolAddress)
    {
        ...
        RiskPoolV2Upgradeable(poolAddress).grantRole(
            RiskPoolV2Upgradeable(poolAddress).ADMIN_ROLE(),
            params.poolOwner
        );
        ...
        emit PoolCreated(
            newPoolId,
            poolAddress,
            RiskPoolV2Upgradeable.PoolType.PRIVATE,
            params.name,
            params.symbol,
            params.poolOwner
        );
        ...
        returns (address poolAddress)
    {
        ...
        emit PoolCreated(
            newPoolId,
            poolAddress,
            RiskPoolV2Upgradeable.PoolType.MUTUAL,
            params.name,
            params.symbol,
            params.poolOwner
        );
    ...
    function _deployPool(
        uint256 poolId,
        RiskPoolV2Upgradeable.PoolConfig memory config
        ...
        RiskPoolV2Upgradeable(poolAddress).grantRole(
            RiskPoolV2Upgradeable(poolAddress).TREASURY_ROLE(),
            treasury
        );

        // Grant DEFAULT_ADMIN_ROLE to factory admin
        RiskPoolV2Upgradeable(poolAddress).grantRole(
            RiskPoolV2Upgradeable(poolAddress).DEFAULT_ADMIN_ROLE(),
            msg.sender
        );
     ...
     * @return Array of pool addresses
     */
    function getPoolsByType(RiskPoolV2Upgradeable.PoolType _poolType)
        external
        view
    ...
    function getPublicPools() external view returns (address[] memory) {
        return poolsByType[RiskPoolV2Upgradeable.PoolType.PUBLIC];
    }
    ...
    function getPrivatePools() external view returns (address[] memory) {
        return poolsByType[RiskPoolV2Upgradeable.PoolType.PRIVATE];
    }
    ...
    function getMutualPools() external view returns (address[] memory) {
        return poolsByType[RiskPoolV2Upgradeable.PoolType.MUTUAL];
    }
    ...
    {
        publicCount = poolsByType[RiskPoolV2Upgradeable.PoolType.PUBLIC].length;
        privateCount = poolsByType[RiskPoolV2Upgradeable.PoolType.PRIVATE].length;
        mutualCount = poolsByType[RiskPoolV2Upgradeable.PoolType.MUTUAL].length;
    }
}
